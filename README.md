...
This program will make only one LED light up at a time from 8 LEDs (pins 0 to 7), and the lit position will move to the next pin every 333 ms.
...
Each time `loop()` is entered, it first checks if `step` exceeds 7. If it does, it resets it to 0.
...
## Hardware
An Arduino NANO development board, a breadboard, some jumpers, 8 LEDs, and 8 220Ω resistors.

### Connection
Complete the breadboard wiring as shown in the diagram below.

![Alt text](media/nano-led-chaser.png)
